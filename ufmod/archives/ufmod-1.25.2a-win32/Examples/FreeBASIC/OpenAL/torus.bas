...
Dim Shared As Integer volume, ang, angx, angy, angz, dist, ttx, tty, OAL_Source
...
Private Function InitOAL() As Integer
...
If ptc_open(SCR_TITLE, SCR_WIDTH, SCR_HEIGHT) <> 0 Then
	If InitOAL() = 1 Then
		...
		If uFMOD_OALPlaySong(Cast(Any Ptr, 1), 0, XM_RESOURCE, OAL_Source) <> 0 Then
			Dim hW As HWND   = GetForegroundWindow()
			Dim d as Integer = -1
