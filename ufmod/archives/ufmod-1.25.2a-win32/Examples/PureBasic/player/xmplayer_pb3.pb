...
;-Global
Global Font1.l, oldseconds.l, vol_oldL.l, vol_oldR.l, timer.l, uaux.l, *hWave.l
Global buf.s, playing.s, playing.s
...
Protected vv.l, vg.l, vm.l
  ...
  ;Affichage des niveaux ssi la valeur est > à 0    
  If value > 0
...
Procedure.l PlayXM()
  ...
  *hWave = uFMOD_PlaySong(theFile, 0, #XM_FILE)
  ...
  Else
    DisableToolBarButton(#But_Open, 1)
    DisableToolBarButton(#But_Stop, 0)
    DisableToolBarButton(#But_Pause, 0)
    ...
    EndIf 
    playing = "Playing "+Chr(34)+XM_Title+Chr(34)
    SetWindowTitle(0, playing)
  EndIf
EndProcedure
...
Procedure StopXM()
  ...
  *hWave = 0
  DisableToolBarButton(#But_Open, 0)
  DisableToolBarButton(#But_Stop, 1)
  DisableToolBarButton(#But_Pause, 1)
  DisableToolBarButton(#But_Play, 1)
...
Procedure PauseXM()
  If *hWave
    uFMOD_Pause()
    DisableToolBarButton(#But_Pause, 1)
    DisableToolBarButton(#But_Play, 0)
  EndIf
EndProcedure
...
;-[Main]
Font1 = LoadFont(0, "Courier", 10)
If OpenWindow(0, 0, 0, 328, 40, #PB_Window_SystemMenu | #PB_Window_ScreenCentered, #TITLE)
  If CreateToolBar(#ToolBar, WindowID())
    CatchImage(#But_Open, ?tbar_open)   : ToolBarImageButton(#But_Open, ImageID())
    ToolBarToolTip(#But_Open, "Open")
    
    CatchImage(#But_Stop, ?tbar_stop)   : ToolBarImageButton(#But_Stop, ImageID())
    ToolBarToolTip(#But_Stop, "Stop")   : DisableToolBarButton(#But_Stop, 1)
    
    CatchImage(#But_Pause, ?tbar_pause) : ToolBarImageButton(#But_Pause, ImageID())
    ToolBarToolTip(#But_Pause, "Pause") : DisableToolBarButton(#But_Pause, 1)
    
    CatchImage(#But_Play, ?tbar_play)   : ToolBarImageButton(#But_Play, ImageID())
    ToolBarToolTip(#But_Play, "Play")   : DisableToolBarButton(#But_Play, 1)
    
    hToolBar.l = GetWindow_(WindowID(), #GW_CHILD)
    SetWindowLong_(hToolBar, #GWL_STYLE, $5000094D)
    MoveWindow_(hToolBar, 5, 10, 95, 50, 0)
  EndIf
  
  If CreateGadgetList(WindowID())
    ImageGadget(#Gadget_VM_0, 280,           4, 0, 0, UseImage(#Vu_Meter_0))
    ImageGadget(#Gadget_VM_1, 280+#_Width+4, 4, 0, 0, UseImage(#Vu_Meter_1))
    
    TrackBarGadget(#TrackBar, 100, 10, 100, 22, 0, 100) : DisableGadget(#TrackBar, 1)
 		SetGadgetState(#TrackBar, 100)
    TextGadget(#Lib_Time, 205, 14, 70, 22, "")
